...
open class Stack: Widget {
    ...
    public func add(_ child: Widget, named name: String, title: String? = nil) {
        ...
        if let title {
            gtk_stack_add_titled(castedPointer(), child.castedPointer(), name, title)
        } else {
            gtk_stack_add_named(castedPointer(), child.castedPointer(), name)
        }
    }
    ...
    public func setVisible(_ child: Widget) {
        if pages.contains(where: { $0 === child }) {
            gtk_stack_set_visible_child(castedPointer(), child.castedPointer())
        }
    }

    public var visibleChild: Widget? {
        guard let childPointer = gtk_stack_get_visible_child(castedPointer()) else {
            return nil
        }
