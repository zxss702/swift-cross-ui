...
struct ToolbarModifier<Child: View>: View, ObservationTrackingView {
    ...
    func computeLayout<Backend: BaseAppBackend>(
        ...
        result.preferences.toolbar = result.preferences.toolbar.overlayed(
            with: resolvedToolbar
        )
        return result
    }
    ...
    private var resolvedToolbar: ResolvedToolbar {
        ResolvedToolbar(items: content().flatMap { $0.resolve() })
    }
}
